...
- (void)updateFerretView
{
	...
	[self.labelTag setText:[NSString stringWithFormat:@" %ld", (long)self.viewSelected.tag]];
	...
	[self.labelNumberOfSiblings setText:[NSString stringWithFormat:@"Index:%ld Views:%ld", (long)[self indexOfSelectedView], (long)[self numberOfSiblingViews]]];

	[self.buttonSubviews setTitle:[NSString stringWithFormat:@"Subviews (%lu)", (unsigned long)[self.viewSelected.subviews count]] forState:UIControlStateNormal];
...
- (NSInteger)numberOfSiblingViews
{
	if (self.viewSelected.superview == nil) {
		return 0;
	}
